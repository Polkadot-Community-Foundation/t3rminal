...
export async function loadAdminQrPayload(): Promise<T3rminalConfigQrPayloadV2 | null> {
  const raw = await getSetting(ADMIN_QR_PAYLOAD_SETTING);
  if (!raw) return null;
  try {
...
/**
 * React hook that mirrors the persisted admin payload. Re-fetches whenever
 * the `settings` table changes (so a fresh scan / clear instantly flows
 * through to every consumer — terminal page, receipt generator, etc.)
 *
 * Returns `undefined` until the first read resolves so callers can
 * distinguish "loading" from "no config".
 */
export function useAdminQrPayload(): T3rminalConfigQrPayloadV2 | null | undefined {
  const [payload, setPayload] = useState<
    T3rminalConfigQrPayloadV2 | null | undefined
  >(undefined);

  useEffect(() => {
    let alive = true;
    const refresh = () => {
      void loadAdminQrPayload().then((p) => {
